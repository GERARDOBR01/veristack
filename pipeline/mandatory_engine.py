...
# Visión lee el NOMBRE visible del gráfico ("Gran Demo", "Gran Demo 40%");
# etapa_activa puede ser el ID de campaña ("gran_demo_pv2026") o solo la
...
def _nucleo_campana(etapa: str) -> list[str]:
    ...
    ≥3 letras ("gran_demo_pv2026" → [gran, demo]; "E1" → [])."""
...
def _corresponde_grafico_etapa(grafico: str, etapa: str) -> Optional[bool]:
    ...
    if set(nucleo) <= set(_tokens_canon(grafico)):
        return True  # "Gran Demo 40%" / "Vive la Gran Demo" ⊇ {gran, demo}
    nucleo_junto = "".join(nucleo)
    if nucleo_junto in g or g in nucleo_junto:
        return True  # variantes sin separadores ("grandemo")
    return False
