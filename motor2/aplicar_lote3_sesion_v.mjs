// aplicar_lote3_sesion_v.mjs — Sesión V (bloque conversacional)
// Aprueba el Lote 3 (p26/27+p35, 18 criterios pendientes): SOLO marca revisado_por_gerardo=true.
// NO toca id, aliases, texto, peso, severidad ni ningún otro campo.
// NO aplica ninguna corrección de golden set (p27 sigue con su peso/severidad original).
...
const objetivo = arr.filter(c => PAGINAS_LOTE3.includes(c.pagina_origen) && c.revisado_por_gerardo !== true);

if (objetivo.length !== ESPERADO) {
  console.error(`ABORTA: esperaba ${ESPERADO} pendientes en p26/27/35, encontré ${objetivo.length}. No se escribe nada.`);
  process.exit(1);
}
...
console.log(`OK: ${tocados} criterios del Lote 3 marcados revisado_por_gerardo=true.`);
